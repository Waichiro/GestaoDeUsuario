...
class User{
    ...
    async findNome(name){
        try{
            var result = await knex.select("*").from("users").where({name: name});
    ...
    async update(id, email, name, role){
        ...
        if(user != undefined){
            ...
            if(name != undefined){
                if(name != user.name){
                    var result = await this.findNome(name);
                    if(result == false){
                        editUser.name = name;
                    }else{
                        return{status: false, err: "O nome ja está cadastrado"};
                    }
                }
            }
    ...
    async delete(id){
        ...
        if(user != undefined){
            ...
            return {status: false, err: "O usuario não esxite entao não pode ser deletado"}
        }
    }
